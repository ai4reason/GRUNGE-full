...
;     (when (equal name "thm_2Ebool_2EFORALL__DEF") (setq *p* p *pr* pr) (break))
     ...
     (format *outputthf0* "~d" (prop-thf0 pr))
	  ...
	  (when c2
	    ...
	    (cond ((and (equal role "type") (> (length name) 3) (equal (subseq name 0 3) "tp_"))
		  ...
		  ((and (equal role "lemma") (> (length name) 5) (equal (subseq name 0 5) "conj_"))
		   (let ((al (format nil "~d(~d,axiom,~d" prefix name (subseq l (+ 2 c1 c2)))))
		     (push al (gethash (subseq name 5) *assumelines*)))
	  ...
	  (if (consp l)
		...
		(CONJECTURE
		 (let ((conjform (process-conj-form (cadr l) (caddr l))))
		   ...
		   (format *outputthf0* "thf(conj_~d,axiom,~d).~%" (cadr l) conjform)))
		(t (break)))
	    (break)))
    (close *f*)
    (close *outputthf0*)
	  ...
	  (if (consp l)
		...
		(CONJECTURE
		 (let ((conjform (process-conj-form (cadr l) (caddr l))))
		   ...
		   (format *outputtff0* "tff(conj_~d,axiom,~d).~%" (cadr l) conjform)))
		(t (break)))
	    (break)))
    (close *f*)
    (close *outputtff0*)
	  ...
	  (if (consp l)
		...
		(CONJECTURE
		 (let ((conjform (process-conj-form (cadr l) (caddr l))))
		   ...
		   (format *outputfof* "fof(conj_~d,axiom,~d).~%" (cadr l) conjform)))
		(t (break)))
	    (break)))
    (close *f*)
    (close *outputfof*)
